...
const EditableCell: FC<EditableCellProps> = ({ cell, onUpdate }) => {
  ...
  return (
    <div
      onPaste={handlePaste}
      className="w-full h-full min-h-20 flex items-center p-3 group transition-colors duration-200 hover:bg-blue-500/10 dark:hover:bg-blue-400/10"
    >
      {isEditing ? (
        <input
          ref={inputRef}
          ...
          aria-label={`编辑单元格 ${cell.id}`}
          className="w-full h-full bg-transparent outline-none focus:ring-2 focus:ring-blue-500 rounded-lg p-0"
        />
      ) : (
        <div
          onClick={() => setIsEditing(true)}
          className="w-full h-full cursor-pointer rounded-lg flex items-center justify-start"
          role="button"
          tabIndex={0}
        >
